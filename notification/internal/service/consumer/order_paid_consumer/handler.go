...
func (s *service) handleOrderPaid(ctx context.Context, msg consumer.Message) error {
	event, err := s.orderPaidDecoder.OrderDecode(msg.Value)
	if err != nil {
	...
	err = s.telegramService.SendOrderPaidNotification(ctx, converter.OrderPaidEventToTemplateData(&event))
	if err != nil {
		logger.Error(ctx, "Failed to send OrderPaid event to telegram", zap.Error(err))
		return err
	}
